...
| 2026-07-29 | Regression checkpoint | `GOCACHE=/tmp/argus-go-build go test ./...`, `go vet ./...`, frontend JavaScript syntax checks, and `git diff --check` passed after the current security, environment, and onboarding work. A fresh `govulncheck ./...` could not fetch the Go advisory index because `vuln.go.dev` returned HTTP 403; no current vulnerability result is claimed. | Complete except external advisory-index access |
