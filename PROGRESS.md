...
| 2026-07-29 | 3q | Added a project/environment-bound private-agent control plane and minimal outbound liveness client. Opaque agent enrollment tokens are hashed at rest; authorized editors can list, create, and revoke agents, with explicit healthy/stale/offline/revoked state derived from each agent's bounded expected interval. The bundled `argus-agent` reports only authenticated liveness/version over HTTPS (loopback HTTP is development-only). The central service receives no reverse-connect or private-network dial capability. Signed work configuration, private result protocol, target execution, offline incidents, and agent UI remain subsequent work. | Complete |
