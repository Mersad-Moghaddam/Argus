...
| 4c | Authenticated project identity and source-selection onboarding | In progress — seven-stage project/source/connection/verification/SLO/notification/summary journey is present; inline source connection, verified-evidence confirmation, and notification integration remain | Current checkpoint |
...
| 2026-07-29 | Regression checkpoint | `GOCACHE=/tmp/argus-go-build go test ./...`, `go vet ./...`, `staticcheck ./...`, frontend JavaScript syntax checks, and `git diff --check` passed after the current security, environment, and onboarding work. A fresh `govulncheck ./...` could not fetch the Go advisory index because `vuln.go.dev` returned HTTP 403; no current vulnerability result is claimed. | Complete except external advisory-index access |
...
| 2026-07-29 | 4c | Replaced new-project polling-first entry with an authenticated, seven-stage project/source/connection/verification/SLO/notification/summary journey. It preserves only non-sensitive project drafts locally, creates no partial project before confirmation, explicitly distinguishes passive telemetry, catalog-only OpenAPI import, disabled synthetic setup, heartbeat, and deferred setup, and directs OpenAPI users to the zero-traffic import wizard. Verification is deliberately shown as pending until Argus has received evidence; no source, SLO, or notification channel is silently enabled. Inline source connection, verified-evidence confirmation, and notification integration remain. | In progress |
...
| 2026-07-29 | 3u | Added an optional fail-closed, Ed25519-signed private-agent configuration envelope. An authenticated enrolled agent receives only its server-bound project/environment identity, heartbeat expectation, key identifier, and 15-minute expiry; no target address, executable work, or reverse connection is issued. The bundled agent verifies the envelope against its provisioned public key before adopting the heartbeat interval. Cryptographic tamper/expiry and API binding coverage passed. | Complete |
| 2026-07-29 | 3v | Added replay-safe private-agent result ingestion. Results are bound to the authenticated agent's project and environment, accept only bounded success/failure evidence, deduplicate each agent/idempotency key, and drive one source-aware failure incident that resolves after a subsequent successful result. API/application tests passed. | Complete |
| 2026-07-29 | 3w | Added durable, project/environment/route-scoped private-agent assignments. Editors create or revoke only bounded GET/HEAD HTTP(S) work; viewers can inspect assignments. A verified signed configuration carries only the enrolled agent's exact-environment assignments, and the bundled outbound-only agent executes them with a bounded timeout, no redirects, no credentials, and a 1 MiB response drain. The authenticated dashboard supports assignment creation, inspection, and revocation. Focused tests, frontend syntax checks, and full Go regression checks passed across the implementation checkpoints. | Complete |
| 2026-07-29 | 3x | Added opt-in OTLP/gRPC Metrics and Trace export on a dedicated listener, sharing the HTTP receiver's credential-bound tenant attribution, scope checks, quota, sanitization, and bounded diagnostics. Startup binds both listeners before serving, and an in-memory generated-client transport test verifies registration, metadata authentication, decoding, and persisted attribution. Full Go regression tests passed (368 checks). | Complete |
