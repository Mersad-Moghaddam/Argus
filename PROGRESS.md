...
| 2026-07-29 | 3q | Added a project/environment-bound private-agent control plane and minimal outbound liveness client. Opaque agent enrollment tokens are hashed at rest; authorized editors can list, create, and revoke agents, while the bundled `argus-agent` reports only authenticated liveness/version over HTTPS (loopback HTTP is development-only). The central service receives no reverse-connect or private-network dial capability. Signed work configuration, private result protocol, target execution, offline incidents, and agent UI remain subsequent work. | Complete |
