...
func NewRuntime(cfg config.Config, processor *appworker.Processor, logger *observability.LogStore) (*Runtime, error) {
	...
	// Broad route polling is retained only as a reversible migration switch.
	// New v2 installs use telemetry, heartbeats, and explicit canaries instead.
	if cfg.RouteBroadPollingEnabled {
